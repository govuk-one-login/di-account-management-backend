...
import {
  ...
  validateUserData,
  deleteUserData,
} from "../delete-user-notifications.js";
...
describe("deleteUserData", () => {
  ...
  test("deletes item from DynamoDB", async () => {
    ...
    expect(dynamoMock).toHaveReceivedCommandWith(DeleteCommand, {
      ...
      Key: { internalCommonSubjectId: TEST_USER_DATA.user_id },
    });
  });
});
  ...
  describe("error handling", () => {
    ...
    test("logs the error message", async () => {
      ...
      expect(errorMessage).toContain(
        "Unable to delete user notifications for message with ID: MessageId, mock error"
      );
    });
  });
});
