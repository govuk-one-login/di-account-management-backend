...
import { handler, mergeTrackerRecords } from "../update-inactive-account-tracker.js";
...
describe("UpdateInactiveAccountTracker handler", () => {
  ...
  test("merges duplicate tracker records and deletes stale duplicate rows", async () => {
    // Two rows for the same user (race condition). The newer activity row (2026-01-02)
    // should win for userLastActive/dateForDeletion; the stale row (2026-01-01) must be deleted.
    dynamoMock.on(QueryCommand).resolves({
      Items: [
        { commonSubjectId: "qwerty", dateForDeletion: "2031-01-01", userLastActive: "2026-01-01T00:00:00.000Z", userLastActiveUpdated: "2026-01-01T00:00:00.000Z", status: "pending", statusLastUpdated: "2026-01-01T00:00:00.000Z", emailAddress: "x", hasSetupMfa: false },
        { commonSubjectId: "qwerty", dateForDeletion: "2031-01-02", userLastActive: "2026-01-02T00:00:00.000Z", userLastActiveUpdated: "2026-01-02T00:00:00.000Z", status: "pending", statusLastUpdated: "2026-01-02T00:00:00.000Z", emailAddress: "x", hasSetupMfa: false },
      ],
    });
    dynamoMock.on(TransactWriteCommand).resolves({});
    ...
    // The event no longer fails - the duplicates are merged instead of erroring.
    expect(result).toEqual<DynamoDBBatchResponse>({ batchItemFailures: [] });

    // The merged record keeps the most-recent userLastActive, and its recomputed
    // dateForDeletion (2031-01-02) matches the newer duplicate row, which is updated in place.
    expect(dynamoMock).toHaveReceivedCommandWith(TransactWriteCommand, {
      TransactItems: expect.arrayContaining([
        expect.objectContaining({
          Put: expect.objectContaining({
            TableName: "test-table",
            Item: expect.objectContaining({ userLastActive: "2026-01-02T00:00:00.000Z", dateForDeletion: "2031-01-02" }),
          }),
        }),
      ]),
    });

    // The stale duplicate row (2031-01-01) is deleted; the row whose dateForDeletion matches
    // the new item (2031-01-02) is updated in place by the Put and must NOT be deleted.
    expect(dynamoMock).toHaveReceivedCommandWith(TransactWriteCommand, {
      TransactItems: expect.arrayContaining([
        expect.objectContaining({
          Delete: expect.objectContaining({ TableName: "test-table", Key: { dateForDeletion: "2031-01-01", commonSubjectId: "qwerty" } }),
        }),
      ]),
    });
    expect(dynamoMock).toHaveReceivedCommandWith(TransactWriteCommand, {
      TransactItems: expect.not.arrayContaining([
        expect.objectContaining({
          Delete: expect.objectContaining({ TableName: "test-table", Key: { dateForDeletion: "2031-01-02", commonSubjectId: "qwerty" } }),
        }),
      ]),
    });
  });
...
describe("mergeTrackerRecords", () => {
  ...
  test("takes email group from the record with the newest emailAddressLastUpdated, independent of activity", () => {
    // The record with the newest email is NOT the one with the newest activity.
